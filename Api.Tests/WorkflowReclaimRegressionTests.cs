...
public class WorkflowReclaimRegressionTests
{
    ...
    [Fact]
    public async Task AttemptCountTracksAllClaims_FailureCountTracksOnlyDomainFailures_StaysConsistentWithTaskAttemptHistory()
    {
        // Пункт фидбэка «Job-счётчик попыток не обновляется вместе с
        // attempt»: после stale-реклейма и retryable-провала job.attempt_count
        // должен совпадать с реальным числом строк task_attempt (согласованность
        // проекций job/attempts), а бюджет retry (failure_count) — считать
        // только настоящие доменные провалы, не реклеймы.
        await using var admin = new NpgsqlConnection(_fixture.SuperuserConnectionString);
        await admin.OpenAsync();
        var (jobId, _, _) = await CreateReadyJobAsync(admin, "counter-consistency");

        // Попытка 1: claim, потом "падает" (stale-реклейм) — доменного
        // провала не было, failure_count не должен вырасти.
        await using var workerA = await OpenAsWorkflowWorkerAsync();
        await ClaimOneAsync(workerA, "worker-a");
        await admin.ExecuteAsync(
            "UPDATE workflow.workflow_job SET lease_until = now() - interval '1 second' WHERE job_id = @jobId",
            new { jobId });

        // Попытка 2 (реклейм): claim, затем настоящий retryable-провал —
        // это первый реальный доменный отказ, failure_count должен стать 1.
        await using var workerB = await OpenAsWorkflowWorkerAsync();
        var second = await ClaimOneAsync(workerB, "worker-b");
        var secondLeaseVersion = second.GetProperty("lease_version").GetInt64();

        await workerB.ExecuteAsync(
            "SELECT workflow.fail_job(@jobId, 'worker-b', @leaseVersion, 'fixture.retryable', true)",
            new { jobId, leaseVersion = secondLeaseVersion });

        // Попытка 3 (после RETRY_WAIT): claim и успешное завершение.
        await admin.ExecuteAsync(
            "UPDATE workflow.workflow_job SET next_attempt_at = now() - interval '1 second' WHERE job_id = @jobId",
            new { jobId });
        await using var workerA2 = await OpenAsWorkflowWorkerAsync();
        var third = await ClaimOneAsync(workerA2, "worker-a");
        var thirdLeaseVersion = third.GetProperty("lease_version").GetInt64();

        await workerA2.ExecuteAsync(
            "SELECT workflow.finish_job(@jobId, 'worker-a', @leaseVersion, 'APPLIED', '{}'::jsonb)",
            new { jobId, leaseVersion = thirdLeaseVersion });

        var jobRow = await admin.QuerySingleAsync(
            "SELECT state, attempt_count, failure_count FROM workflow.workflow_job WHERE job_id = @jobId",
            new { jobId });
        var taskAttemptCount = await admin.QuerySingleAsync<int>(
            "SELECT count(*) FROM workflow.task_attempt WHERE job_id = @jobId",
            new { jobId });

        Assert.Equal("SUCCEEDED", (string)jobRow.state);
        // 3 физических claim'а (1 stale-реклейм + 2 реальных) — attempt_count
        // и число строк task_attempt обязаны совпадать.
        Assert.Equal(3, (int)jobRow.attempt_count);
        Assert.Equal(3, taskAttemptCount);
        Assert.Equal((int)jobRow.attempt_count, taskAttemptCount);
        // Но только ОДИН из трёх claim'ов был настоящим доменным провалом —
        // failure_count должен отражать именно это, а не физическое число попыток.
        Assert.Equal(1, (int)jobRow.failure_count);
    }

    [Fact]
    public async Task TaskAttempt_DuplicateAttemptNumberForSameJob_IsRejectedByUniqueConstraint()
    {
        // Прямая проверка unique constraint из миграции 011
        // (task_attempt_job_attempt_number_key) — защита от регрессии в
        // самой логике нумерации attempt'ов, а не только доказательство
        // текущего отсутствия бага.
        await using var admin = new NpgsqlConnection(_fixture.SuperuserConnectionString);
        await admin.OpenAsync();
        var (jobId, executionId, _) = await CreateReadyJobAsync(admin, "unique-constraint");

        await using var workerA = await OpenAsWorkflowWorkerAsync();
        await ClaimOneAsync(workerA, "worker-a");

        var exception = await Assert.ThrowsAsync<PostgresException>(() => admin.ExecuteAsync(
            @"INSERT INTO workflow.task_attempt (job_id, execution_id, lease_version, attempt_number, status)
              VALUES (@jobId, @executionId, 99, 1, 'RUNNING')",
            new { jobId, executionId }));

        Assert.Equal("23505", exception.SqlState); // unique_violation
    }

    // Отдельный сетап под workflow.receive_signal/apply_signal: процесс,
    // стоящий на WAIT_SIGNAL прямо сейчас (минуя automatic-шаг/claim —
    // тестируем сигнальный контракт отдельно от lease/fencing выше).
    private static async Task<Guid> CreateWaitingSignalProcessAsync(
        NpgsqlConnection superuserConnection, string suffix, string signalType)
    {
        var flowName = $"signal-test-{suffix}";
        var processId = Guid.NewGuid();
        var stepInstanceId = Guid.NewGuid();

        await superuserConnection.ExecuteAsync(
            "INSERT INTO workflow.flow_definition (flow_name) VALUES (@flowName)", new { flowName });
        await superuserConnection.ExecuteAsync(
            @"INSERT INTO workflow.flow_version (flow_name, flow_version, status, is_active, map_definition)
              VALUES (@flowName, 1, 'PUBLISHED', true, '{}'::jsonb)", new { flowName });
        await superuserConnection.ExecuteAsync(
            @"INSERT INTO workflow.step_definition
                  (flow_name, flow_version, step_key, step_type, is_start, wait_signal_type, declared_outcome)
              VALUES (@flowName, 1, 'wait_here', 'WAIT_SIGNAL', true, @signalType, 'RECEIVED')",
            new { flowName, signalType });
        await superuserConnection.ExecuteAsync(
            @"INSERT INTO workflow.step_definition (flow_name, flow_version, step_key, step_type, is_start)
              VALUES (@flowName, 1, 'done', 'END', false)", new { flowName });
        await superuserConnection.ExecuteAsync(
            @"INSERT INTO workflow.transition_definition (flow_name, flow_version, step_key, outcome, next_step_key)
              VALUES (@flowName, 1, 'wait_here', 'RECEIVED', 'done')", new { flowName });
        await superuserConnection.ExecuteAsync(
            @"INSERT INTO workflow.process_instance
                  (process_id, business_key, flow_name, flow_version, state, current_step_key, process_data)
              VALUES (@processId, @businessKey, @flowName, 1, 'WAITING_SIGNAL', 'wait_here', '{}'::jsonb)",
            new { processId, businessKey = $"biz-{suffix}", flowName });
        await superuserConnection.ExecuteAsync(
            @"INSERT INTO workflow.step_instance (step_instance_id, process_id, step_key, step_type, state)
              VALUES (@stepInstanceId, @processId, 'wait_here', 'WAIT_SIGNAL', 'WAITING')",
            new { stepInstanceId, processId });
    ...
    [Fact]
    public async Task ReceiveSignal_DuplicateMessageId_ReturnsDuplicateWithoutReapplying()
    {
        // "Повтор сигнала" из фидбэка: тот же message_id/process/type/тело
        // второй раз не должен ни завести вторую строку в workflow_signal,
        // ни повторно продвинуть процесс — идемпотентность на message_id.
        await using var superuser = new NpgsqlConnection(_fixture.SuperuserConnectionString);
        await superuser.OpenAsync();
        var processId = await CreateWaitingSignalProcessAsync(superuser, "duplicate", "test.signal");

        var body = "{\"x\": 1}";
        var first = await superuser.ExecuteScalarAsync<string>(
            "SELECT workflow.receive_signal(@processId, 'msg-1', 'test.signal', @body::jsonb)::text",
            new { processId, body });
        var firstResult = JsonDocument.Parse(first!).RootElement;
        Assert.Equal("accepted", firstResult.GetProperty("signalStatus").GetString());

        var processAfterFirst = await superuser.QuerySingleAsync<string>(
            "SELECT state FROM workflow.process_instance WHERE process_id = @processId", new { processId });
        Assert.Equal("COMPLETED", processAfterFirst);

        var second = await superuser.ExecuteScalarAsync<string>(
            "SELECT workflow.receive_signal(@processId, 'msg-1', 'test.signal', @body::jsonb)::text",
            new { processId, body });
        var secondResult = JsonDocument.Parse(second!).RootElement;
        Assert.Equal("ok", secondResult.GetProperty("status").GetString());
        Assert.Equal("duplicate", secondResult.GetProperty("signalStatus").GetString());

        var signalRows = await superuser.QuerySingleAsync<int>(
            "SELECT count(*) FROM workflow.workflow_signal WHERE message_id = 'msg-1'");
        Assert.Equal(1, signalRows);

        var processAfterSecond = await superuser.QuerySingleAsync<string>(
            "SELECT state FROM workflow.process_instance WHERE process_id = @processId", new { processId });
        Assert.Equal("COMPLETED", processAfterSecond);
    }

    [Fact]
    public async Task ReceiveSignal_ConflictingMessageId_RejectedWithoutOverwritingOriginal()
    {
        // "Конфликт сигнала" из фидбэка: тот же message_id, но другое тело —
        // должно быть отклонено как workflow.signal_conflict, не перезаписывая
        // и не удаляя исходную (первую) принятую запись.
        await using var superuser = new NpgsqlConnection(_fixture.SuperuserConnectionString);
        await superuser.OpenAsync();
        var processId = await CreateWaitingSignalProcessAsync(superuser, "conflict", "test.signal");

        var originalBody = "{\"x\": 1}";
        await superuser.ExecuteAsync(
            "SELECT workflow.receive_signal(@processId, 'msg-conflict', 'test.signal', @body::jsonb)",
            new { processId, body = originalBody });

        var conflictingBody = "{\"x\": 999}";
        var conflict = await superuser.ExecuteScalarAsync<string>(
            "SELECT workflow.receive_signal(@processId, 'msg-conflict', 'test.signal', @body::jsonb)::text",
            new { processId, body = conflictingBody });
        var conflictResult = JsonDocument.Parse(conflict!).RootElement;
        Assert.Equal("error", conflictResult.GetProperty("status").GetString());
        Assert.Equal("workflow.signal_conflict", conflictResult.GetProperty("code").GetString());

        var storedBody = await superuser.QuerySingleAsync<string>(
            "SELECT body::text FROM workflow.workflow_signal WHERE message_id = 'msg-conflict'");
        Assert.Equal(JsonDocument.Parse(originalBody).RootElement.GetRawText(),
            JsonDocument.Parse(storedBody).RootElement.GetRawText());
    }
}
